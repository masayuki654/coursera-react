...
import NavigateToTimer from './NavigateToTimer';
...
class App extends React.Component{
    ...
    */
    ...
    render() {
        const isRunning = this.timer.pomodoro.timerState === "Running";
        const isPomodoro = this.timer.pomodoro.type === "Pomodoro";
        const timeLeftDisplay = TransferTime(this.timer.pomodoro.timeLeft);
            ...
            <header>Pomodoro</header>
                <div className="pomodoro-timer">
                    <div className="mode-select-button">
                        <button onClick={NavigateToTimer} style={ isPomodoro ? {pointerEvents: "none", color: "blue"} : null }>Pomodoro</button>    
                        <button onClick={NavigateToTimer} style={ !isPomodoro ? {pointerEvents: "none", color: "blue"} : null }>Short Break</button>
                    </div>
                    ...
                    <div className="running">
                        <button onClick={StopTimer}>stop</button><button onClick={EndTimer}>end</button> 
                    </div>:
                    <button onClick={StartTimer}>start</button>}
                </div>

                <div className="message">{this.timer.pomodoro.type === "Pomodoro" ? this.timer.pomodoro.message : this.timer.shortBreak.message}</div>
        </div>
        );
    }
}
